...
//        let patImage: UIImage = UIImage(named: "pattern.png")!
//        let woody = UIColor.init(patternImage: patImage)
...
//        woody.setFill()
        ...
        let lengthTick2 = addLine(CGPoint(x: run, y: newY - picketHeight - 45),
                                  finish: CGPoint(x: maxX/2 + 10, y: postSpaceHeight - ySpace))
        lengthTick2.stroke()
        addArrow(CGPoint(x: run, y: newY - picketHeight - 45), angle: 180 - angle)
        ...
        /**************************/
        if spindles > 1 {
            let midXOffset = (spindle2Location.x - spindle1Location.x)/2
            ...
            spaceTick1.stroke()
            addArrow(pointShift(spindle1Location, xshift: 3, yshift: -25), angle: angle)
            ...
            spaceTick2.stroke()
            addArrow(pointShift(spindle2Location, xshift: -3, yshift: -25), angle: 180 - angle)
        ...
        if angle > 0 {
            ...
            riseMeasure1.stroke()
            addArrow(CGPoint(x: maxX + 10, y: newY), angle: 90)
            ...
            riseMeasure2.stroke()
            addArrow(CGPoint(x: maxX + 10, y: yOrigin), angle: 270)
            ...
            let runTick2 = addLine(CGPoint(x: run, y: yOrigin + 15),
                                   finish: CGPoint(x: maxX/2 + 10, y: yOrigin + 15))
            runTick2.stroke()
            addArrow(CGPoint(x: run, y: yOrigin + 15), angle: 180)
            ...
            addArrow(CGPoint(x: xOrigin + maxX/1.3, y: yOrigin), angle: 265)
            
            let angleMeasure2 = UIBezierPath(arcCenter: CGPoint(x: xOrigin, y: yOrigin),
                                            radius: maxX / 1.3,
                                            startAngle: -angle / 180 * π * 0.90,
                                            endAngle: -angle / 180 * π * 0.6,
                                            clockwise: true)
            angleMeasure2.lineWidth = 1
            angleMeasure2.stroke()
            let arrowXOrigin: CGFloat = xOrigin + maxX/1.3
            addArrow(CGPoint(x: 2 + cos(angle * π / 180) * arrowXOrigin, y: yOrigin + 8 - sin(angle * π / 180) * arrowXOrigin), angle: 70)
        }
      }
